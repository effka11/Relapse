	...
	if mc == MOUSE_LEFT then
		...
		if hoveredskill then
			...
			elseif hoveredskill == -1 then
				self:DisplayMessage("You need to be level "..GAMEMODE.MaxLevel.." to remort!", COLOR_RED)
...
function GM:DrawXPBar(x, y, w, h, xpw, barwm, hm, level)
	...
	RelapseUI.PaintHudHairBar(x, y + row, barw, barh, progress, c.Accent)

	local sp = MySelf:GetZSSPRemaining()
	local left = (level == GAMEMODE.MaxLevel) and translate.Get("hud_level_max") or translate.Format("hud_level", level)
	left = left .. append
	if sp > 0 then
		left = left .. "  ·  " .. sp .. " SP"
	end
	RelapseUI.HudText(left, "Relapse17", x, y + row * 0.5, (level == GAMEMODE.MaxLevel or sp > 0) and c.Accent or c.Muted, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)

	if level ~= GAMEMODE.MaxLevel then
		RelapseUI.HudText(string.CommaSeparate(xp) .. " / " .. string.CommaSeparate(GAMEMODE:XPForLevel(level + 1)), "Relapse17", x + barw, y + row * 0.5, c.Text, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
	end
end
