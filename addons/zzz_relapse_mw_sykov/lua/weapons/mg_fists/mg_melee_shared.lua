...
-- Melee camera bone is 1:1 on the eye. Fists walk/sprint move that bone
-- harder than a gun VM; scale each state so the world bob matches a rifle.
if CLIENT then
	local function IsMeleeCameraTask(wep)
		local tasks = wep.Tasks
		if not tasks or not wep.GetCurrentTask then
			return false
		end
		local task = tasks[wep:GetCurrentTask()]
		local name = task and task.Name
		return isstring(name) and string.StartWith(name, "Melee")
	end

	local function CameraScale(wep)
		if IsMeleeCameraTask(wep) then
			return wep.RelapseMeleeCameraScale or 0.10
		end
		if wep.HasFlag and wep:HasFlag("Sprinting") then
			if wep:HasFlag("SuperSprint") then
				return wep.RelapseSuperSprintCameraScale or wep.RelapseSprintCameraScale or 1
			end
			return wep.RelapseSprintCameraScale or 1
		end
		return wep.RelapseWalkCameraScale or 1
	end
