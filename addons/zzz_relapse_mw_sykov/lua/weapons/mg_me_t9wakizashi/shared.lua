...
SWEP.RelapsePreviewHullBounds = false
SWEP.RelapsePreviewParts = {
	"models/easy/cw/weapons/wm_me_t9wakizashi.mdl",
}
-- Hull +X like PKM, not the machete's +Z. Yaw 90 lays the blade in profile;
-- roll 45 around the blade puts the edge on the card.
SWEP.RelapsePreviewAngle = Angle(0, 90, 0)
SWEP.RelapsePreviewLocalAng = Angle(0, 0, 45)
...
SWEP.PrintName = "Вакидзаси"
SWEP.Description = "A short sword. Light steel: it cuts the one behind the doorway, not the one in the next room."
SWEP.TranslationName = "wep_cwwakizashi"
SWEP.TranslationDescription = "wep_cwwakizashi_desc"
SWEP.Category = "Black Ops: Cold War"
SWEP.SubCategory = "Melees"
SWEP.Spawnable = true
SWEP.VModel = Model("models/easy/cw/weapons/vm_me_t9wakizashi.mdl")
SWEP.WorldModel = Model("models/easy/cw/weapons/wm_me_t9wakizashi.mdl")
SWEP.Purpose = "A short sword. Light steel: it cuts the one behind the doorway, not the one in the next room."
...
-- Stopping is knockback. Range is hull trace hu. Weight is kilograms (wakizashi ~0.55 kg).
-- Four hits on 225, not CoD 75. Faster slash than the T3 machete: behind the doorway, not three cuts.
SWEP.Relapse = {
	Melee = true,
	Type = "slash",
	Damage = 57,
